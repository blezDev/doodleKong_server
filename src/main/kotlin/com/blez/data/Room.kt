package com.blez.data

import com.blez.data.models.*
import com.blez.gson
import com.blez.other.getRandomWords
import com.blez.other.matchesWord
import com.blez.other.transformToUnderscores
import com.blez.other.words
...
class Room(
    ...
    fun removePlayer(clientId : String ){
        ...
        val announcement = Announcement("${player.username} left the party :(",System.currentTimeMillis(),
...
private suspend fun sendWordToPlayer(player: Player){
    ...
    word?.let {curWord->
        drawingPlayer?.let {drawingPlayer->
           val gameState = GameState(drawingPlayer.username,
           if(player.isDrawing || phase == Phase.SHOW_WORD ) curWord else curWord.transformToUnderscores()
           )
    ...
    private fun waitingForPlayers() {
        GlobalScope.launch {
            val phaseChange = PhaseChange(Phase.WAITING_FOR_PLAYERS,
    ...
    private fun waitingForStart() {
        ...
        GlobalScope.launch {
            val phaseChange = PhaseChange(Phase.WAITING_FOR_START,
                DELAY_WAITING_FOR_START_TO_NEW_ROUND)
            broadcast(gson.toJson(phaseChange))
        }
    ...
    private fun gameRunning() {
        ...
        GlobalScope.launch {
            broadcastToAllExcept(gson.toJson(gameStateForGuessingPlayer),drawingPlayer?.clientId ?: players.random().clientId)
            drawingPlayer?.socket?.send(Frame.Text(gson.toJson(gameStateForDrawingPlayer)))

            timeAndNotify(DELAY_GAME_RUNNING_TO_SHOW_WORD)
            println("Drawing phase in room $name started. It'll last ${DELAY_GAME_RUNNING_TO_SHOW_WORD}")
